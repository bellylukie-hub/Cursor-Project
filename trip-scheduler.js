...
(function () {
    ...
    function getDriverById(id) {
        return schedulerDrivers.find(d => d.id === id) || (typeof getFleetOrderDashboardStats !== 'undefined' ? null : null);
    }
    ...
    function renderTripForm(trip) {
        ...
        const orderOpts = schedulerOrders.map(o =>
            `<option value="${o.id}">${o.orderNumber} — ${o.origin} → ${o.destination} (${o.cargoType || ''})</option>`
        ).join('') || '<option value="">No orders available</option>';
        ...
        return `
            <div class="trip-scheduler-form">
                ...
                <div class="trip-order-entry" style="margin:20px 0;padding:16px;background:#f7fafc;border-radius:8px;border:1px solid var(--border);">
                    <h4 style="margin:0 0 12px;">Add Order to Trip</h4>
                    <div class="form-grid-2">
                        <div class="form-group"><label>Order Number</label><select class="form-control" id="tsAddOrderId" onchange="onTripOrderSelect()">${orderOpts}</select></div>
                    ...
                    <div class="form-group"><label>Remarks</label><input class="form-control" id="tsAddRemarks"></div>
                ...
                <div class="table-container" style="box-shadow:none;">
                    ...
                    <table class="client-orders-grid" style="min-width:1200px;">
                        <thead><tr>
                            <th>Sl</th><th>Order No</th><th>From / To</th><th>Cargo Type</th><th>Container No</th>
                            <th>Entry Border</th><th>Via 1</th><th>Via 2</th><th>Seal</th><th>Trailer Pos</th><th></th>
                        </tr></thead>
                        <tbody id="tsOrdersBody">
                            ${orders.length ? orders.map((to, i) => `<tr>
                                <td>${to.slNo || i + 1}</td>
                                <td><strong>${to.orderNumber || '—'}</strong></td>
                                <td>${to.fromStation || '—'} → ${to.toStation || '—'}</td>
                                <td>${to.cargoType || '—'}</td>
                                <td>${to.containerNo || '—'}</td>
                            ...
                            </tr>`).join('') : '<tr><td colspan="11" style="text-align:center;padding:16px;color:var(--text-secondary);">No orders added — select an order above and click Add to Trip.</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>`;
    ...
    window.onTripOrderSelect = function () {
        const order = getOrderById(document.getElementById('tsAddOrderId')?.value);
        if (!order) return;
        document.getElementById('tsAddCargoType').value = order.cargoType || '';
        document.getElementById('tsAddFrom').value = order.origin || '';
        document.getElementById('tsAddTo').value = order.destination || '';
        document.getElementById('tsAddEntryBorder').value = order.entryBorder || '';
        document.getElementById('tsAddVia1').value = order.viaBorder1 || '';
        document.getElementById('tsAddVia2').value = order.viaBorder2 || '';
    };

    window.addOrderToTripDraft = function () {
        const orderId = document.getElementById('tsAddOrderId')?.value;
        const order = getOrderById(orderId);
        if (!order) { showToast('Select an order', 'warning'); return; }
        ...
        const containerNo = (ld.containerLines || [])[0]?.containerNo || '';
        draftTripOrders.push({
            ...
            containerNo, trailerPosition: trailerPos,
        ...
        refreshTripFormBody();
        showToast('Order added to trip', 'success');
    };
    ...
    window.saveTripScheduler = async function () {
        ...
        try {
            ...
            if (saved) {
                ...
                tripsDB.unshift(payload);
            }
            saveLocal();
            currentTripId = null;
            draftTripOrders = [];
            showToast('Trip saved and orders scheduled', 'success');
            ...
            <div class="table-container" style="margin-top:24px;">
                ...
                <table class="data-table">
                    <thead><tr>
                        ...
                        <th>Driver</th><th>Orders</th><th>Status</th>${edit ? '<th></th>' : ''}
                    </tr></thead>
                    <tbody>
